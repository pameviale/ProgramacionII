...
fun listaSegsMedianoche(tlist: tiempo list) = if (tiempoCorrecto(hd(tlist))) then segsMedianoche(hd(tlist))::listaSegsMedianoche(tl(tlist)) else (~1)::listaSegsMedianoche(tl(tlist));
