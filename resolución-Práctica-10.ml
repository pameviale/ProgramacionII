...
fun f x = 1
fun suma(x,y) = x+y
fun cant_estudiantes_no_inscriptos(lista, c) = (foldr suma 1 (map f (estudiantes_no_inscriptos(lista,c))));

(* C *)

fun estudiantes_no_inscriptosv2(lista: estudiante list, c:string) =
let 
	val l = filter (no_inscripto c) lista
in
	map nom l
end;
...
(* D *)
fun cant_estudiantes_no_inscriptosv2(lista, c) = 
let 
	fun f x = 1;
	fun suma(x,y) = x+y;
	val lista_est = estudiantes_no_inscriptosv2(lista,c)
in 
	foldr suma 1 (map f lista_est)
end;
